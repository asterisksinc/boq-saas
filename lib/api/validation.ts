...
export const proposalCreateSchema = z.object({
  ...
  sourceType: z.enum(["scratch", "boq", "duplicate", "template"]).default("scratch"),
  sourceId: optionalNullableUuid,
  sourceLabel: z.string().trim().min(1).max(240).nullable().optional(),
  proposedValue: z.coerce.number().finite().min(0).max(9999999999999999),
  expiryDate: isoDate.nullable().optional(),
  internalNotes: z.string().trim().max(5000).nullable().optional(),
  scopeItems: z.array(z.string().trim().min(1).max(500)).max(100).optional(),
}).strict().superRefine((value, ctx) => {
  if (value.sourceType !== "scratch" && !value.sourceId) {
    ctx.addIssue({ code: "custom", path: ["sourceId"], message: "sourceId is required for this source type." });
  }
});
...
export const proposalPatchSchema = z.object({
  ...
  proposedValue: z.coerce.number().finite().min(0).max(9999999999999999).optional(),
  expiryDate: isoDate.nullable().optional(),
  internalNotes: z.string().trim().max(5000).nullable().optional(),
  scopeItems: z.array(z.string().trim().min(1).max(500)).max(100).optional(),
}).strict();
...
export const proposalStatusSchema = z.object({
  status: z.enum(["draft", "sent", "approved", "revisions", "won", "lost"]),
}).strict();
...
export const invoiceStatusSchema = z.object({
  status: z.enum(["draft", "pending", "sent", "accepted", "void"]),
}).strict();

export const paymentCreateSchema = z.object({
  amount: z.coerce.number().finite().positive().max(9999999999999999),
  paidAt: z.string().datetime({ offset: true }).optional(),
  method: z.enum(["cash", "bank_transfer", "card", "upi", "cheque", "other"]),
  reference: z.string().trim().max(200).nullable().optional(),
